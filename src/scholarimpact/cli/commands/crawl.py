...
@click.option('--delay-min', default=5.0, type=float, help='Minimum delay between requests (default: 5.0)')
@click.option('--delay-max', default=10.0, type=float, help='Maximum delay between requests (default: 10.0)')
@click.option('--delay-between-articles-min', default=16.0, type=float, help='Minimum delay between articles (default: 16.0)')
@click.option('--delay-between-articles-max', default=22.0, type=float, help='Maximum delay between articles (default: 22.0)')
@click.option('--output-dir', help='Output directory (defaults to author.json directory)')
def crawl_citations(author_json, openalex_email, max_citations, delay_min, delay_max, delay_between_articles_min, delay_between_articles_max, output_dir):
        ...
        for article in article_bar:
            ...
            try:
                ...
                citations = crawler.crawl_all_citations(
                    cites_id,
                    max_pages=None  # max_citations is handled differently
                )
