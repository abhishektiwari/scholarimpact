...
def get_asset_path(asset_name: str) -> Optional[Path]:
    ...
    try:
        ...
        asset_path = pkg_resources.resource_filename('scholarimpact.assets', asset_name)
...
def copy_fonts(output_dir: str) -> int:
    ...
    license_files = []
    if fonts_dir.exists():
        font_files = list(fonts_dir.glob("*.ttf")) + list(fonts_dir.glob("*.otf")) + list(fonts_dir.glob("*.woff*"))
