...
class StreamlitAppComponent(BaseComponent):
    ...
    def validate_data(self, data: Dict[str, Any]) -> bool:
        ...
        return (
            data.get('author') is not None and
            data.get('articles') is not None and 
            not data['articles'].empty
        )
    ...
    def render(self, data: Dict[str, Any], data_dir: str = 'data', **kwargs) -> None:
        ...
        def _render_app():
            ...
            THEME_COLORS = st.get_option("theme.chartCategoricalColors") or ["#0ea5e9", "#059669", "#fbbf24"]
            ...
            COLOR_PALETTE = {
                ...
                'gradient': ['#e6fef5', '#b8f5e0', '#8aeccc', '#5ce3b7', '#2dd4a3', '#0dc58e', '#059669'],
            ...
            if sort_options[sort_by] in pub_analysis.columns:
                pub_analysis_sorted = pub_analysis.sort_values(sort_options[sort_by], ascending=ascending)
            else:
            ...
            for _, row in pub_analysis_sorted.iterrows():
                ...
                display_title = f"{title[:60]}{'...' if len(title) > 60 else ''} ({citations} citations)"
            ...
            current_selection_index = display_titles.index(selected_display) if selected_display in display_titles else 0
            if current_selection_index != st.session_state.selected_article_index and not url_fragment:
            ...
            if selected_display:
                ...
                pub_data = pub_analysis_sorted[pub_analysis_sorted['title'] == selected_title].iloc[0]
    ...
    def _display_author_info(self, author_data: Dict[str, Any], show_interests: bool = False):
        ...
        with col1:
            if author_data.get('scholar_profile_url'):
                st.link_button("Scholar", author_data['scholar_profile_url'], icon=":material/open_in_new:")
    ...
    def _prepare_articles_data(self, articles_df: pd.DataFrame, data_dir: str = 'data') -> pd.DataFrame:
        ...
        df_clean['total_citations'] = pd.to_numeric(df_clean['total_citations'], errors='coerce').fillna(0)
            ...
            if 'cites_id' in row and row['cites_id']:
                ...
                if os.path.exists(json_file):
                    try:
                        ...
                        for citation in citations_data:
                            ...
                            for author_details in citing_authors_details:
                                if isinstance(author_details, dict):
                                    ...
                                    institution_name = author_details.get('institution_display_name', '') or ''
                                    
                                    country = country.strip() if country else ''
                                    
                                    if country and country not in ['Unknown', 'Invalid', ''] and country not in processed_countries:
                        ...
                        if citing_countries:
                            df_clean.at[idx, 'unique_countries_from_crawler'] = len(citing_countries)
                            
                            try:
                                df_clean.at[idx, 'top_countries_from_affiliations'] = citing_countries
                                df_clean.at[idx, 'country_locations'] = country_locations if country_locations else []
                            except (ValueError, TypeError) as assign_error:
                                print(f"Warning: Could not assign complex data for {file_cites_id}: {assign_error}")
                        ...
                        if 'top_countries_from_affiliations' not in df_clean.columns or pd.isna(df_clean.at[idx, 'top_countries_from_affiliations']):
                            df_clean.at[idx, 'top_countries_from_affiliations'] = {}
                        if 'country_locations' not in df_clean.columns or pd.isna(df_clean.at[idx, 'country_locations']):
                            df_clean.at[idx, 'country_locations'] = []
                else:
                    df_clean.at[idx, 'crawler_data_available'] = False
    ...
    def _render_main_content(self, pub_data: pd.Series, COLOR_PALETTE: Dict[str, Any], data_dir: str):
        ...
        has_enhanced_geo_data = ('top_countries_from_affiliations' in pub_data and 
                                pub_data['top_countries_from_affiliations'] and
                                isinstance(pub_data['top_countries_from_affiliations'], dict))
        ...
        if has_enhanced_geo_data:
            ...
            with col2:
                if 'unique_citing_authors_from_crawler' in pub_data and pd.notna(pub_data['unique_citing_authors_from_crawler']):
                    st.metric("Citing Authors", f"{int(pub_data['unique_citing_authors_from_crawler']):,}")
        ...
        if citations_table_data and len(citations_table_data) > 0:
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "Author Affiliations": st.column_config.TextColumn("Affiliations", width="large", help="Institutions of citing authors"),
                    "Countries": st.column_config.TextColumn("Countries", width="medium", help="Countries of citing authors")
                }
            )
            
            st.info(f"Showing {len(citations_df)} citing papers with detailed author information")
        else:
            st.info("No detailed citation data available. Run the citation crawler to generate detailed citation information.")
    ...
    def _generate_insight(self, pub_data: pd.Series) -> str:
        ...
        if ('top_countries_from_affiliations' in pub_data and 
            pub_data['top_countries_from_affiliations'] and 
            isinstance(pub_data['top_countries_from_affiliations'], dict)):
            unique_countries = len(pub_data['top_countries_from_affiliations'])
        elif 'unique_countries_from_crawler' in pub_data and pd.notna(pub_data['unique_countries_from_crawler']):
        ...
        for citation in citations_data:
            ...
            for author_details in citing_authors_details:
                if isinstance(author_details, dict):
                    ...
                    if (institution and 
                        institution not in ['Unknown', 'Invalid', ''] and 
                        institution not in processed_institutions):
    ...
    def _render_visualizations(self, pub_data: pd.Series, COLOR_PALETTE: Dict[str, Any], data_dir: str, has_enhanced_geo_data: bool):
        ...
        if countries_data:
            ...
            bar_colors = [COLOR_PALETTE['chart_colors'][i % len(COLOR_PALETTE['chart_colors'])] for i in range(len(countries))]
        ...
        if has_enhanced_geo_data:
            ...
            if year_data:
                ...
                st.caption(f"Total citations with year data: {total_with_years} citations across {len(years)} years (avg: {avg_per_year:.1f} per year)")
            else:
        ...
        if domain_data and any(domain_data.values()):
            ...
            with domain_tab:
                if domain_data.get('domains'):
                    ...
                    fig = go.Figure(data=[go.Pie(
                        ...
                        hole=0.3,
                        marker=dict(
                            colors=COLOR_PALETTE['qualitative'][:len(domains)]
                        ),
                    ...
                    st.caption(f"Papers distributed across {len(domains)} research domains (Total: {total_with_domains} papers with domain data)")
                else:
            ...
            with field_tab:
                if domain_data.get('fields'):
                    ...
                    sorted_fields = dict(sorted(fields.items(), key=lambda x: x[1], reverse=True)[:15])
                    
                    # Cycle through theme colors for each bar
                    field_colors = [COLOR_PALETTE['chart_colors'][i % len(COLOR_PALETTE['chart_colors'])] for i in range(len(sorted_fields))]
                    ...
                    st.caption(f"Citations from {total_fields} different fields (avg: {avg_per_field:.1f} citations per field)")
                else:
            ...
            with subfield_tab:
                if domain_data.get('subfields'):
                    ...
                    sorted_subfields = dict(sorted(subfields.items(), key=lambda x: x[1], reverse=True)[:20])
                    ...
                    if sorted_subfields:
                        ...
                        st.caption(f"Total of {total_subfields} unique research subfields identified")
                    else:
            ...
            if domain_data.get('domains') and domain_data.get('fields'):
                ...
                with col3:
                    ...
                    if total > 0:
                        entropy = -sum((v/total) * math.log(v/total) for v in field_values if v > 0)
                        normalized_entropy = entropy / math.log(len(field_values)) if len(field_values) > 1 else 0
            ...
            st.info("Research domain analysis requires citation data with OpenAlex primary_topic information. Re-crawl with OpenAlex enabled to see this analysis.")
    ...
    def _analyze_research_domains(self, pub_data: pd.Series, data_dir: str) -> Dict[str, Dict[str, int]]:
    ...
    def _create_citations_table(self, pub_data: pd.Series, data_dir: str) -> List[Dict[str, Any]]:
        ...
        for citation in citations_data:
            ...
            if isinstance(citing_authors_details, list):
                for author_details in citing_authors_details:
                    if isinstance(author_details, dict):
                        ...
                        if affiliation and affiliation not in ['Unknown', 'Invalid', ''] and affiliation not in affiliations_list:
                            affiliations_list.append(affiliation)
                        
                        if country and country not in ['Unknown', 'Invalid', ''] and country not in countries_list:
            ...
            citation_row = {
                ...
                'Author Affiliations': ', '.join(sorted(set(affiliations_list))) if affiliations_list else 'Unknown',
                'Countries': ', '.join(sorted(set(countries_list))) if countries_list else 'Unknown'
            }
